...
class TrafficViolationSystem:
    ...
    def process_video(self):
        ...
        try:
            ...
            while True:
                ...
                # IMPORTANT: Keep original frame for high-quality VLM processing
                original_frame = frame.copy()
                
                # Calculate scale factor for coordinate conversion
                scale_factor = self._calculate_scale_factor(frame)
                
                # Resize frame for detection (performance optimization)
                frame = self._resize_frame_if_needed(frame)
                
                # Process the current frame
                try:
                    processed_frame, frame_detections, new_violations = self.process_frame(
                        frame, frame_count, original_frame, scale_factor
                    )
    ...
    def _calculate_scale_factor(self, frame):
        ...
        # Check max dimension scaling
        if max(original_height, original_width) > self.args.max_dimension:
            scale_factor = max(original_height, original_width) / self.args.max_dimension
    ...
    def process_frame(self, frame, frame_count, original_frame=None, scale_factor=(1.0, 1.0)):
        """
        Process a single video frame with HSV traffic light detection
        
        Args:
            frame: The resized frame for detection processing
            frame_count: Current frame count
            original_frame: Original high-quality frame for VLM processing
            scale_factor: Scale factor tuple (scale_x, scale_y) for coordinate conversion
            ...
            for det in detections:
                ...
                for detector in self.violation_detectors:
                    ...
                    if violated:
                        ...
                        # Add vehicle_class to violation_info
                        violation_info['vehicle_class'] = int(det["label"])
                        
                        self.violations[track_id] = violation_info
                        if self.args.debug:
                            print(f"🚨 VIOLATION DETECTED! Vehicle {track_id}, frame {frame_count}, " 
                                f"type: {violation_info.get('violation_type', 'unknown')}, class: {det['label']}")
                        
                        # Add to VLM processing with HIGH-QUALITY original frame
                        bbox = det["box"]
                        ...
                        # Scale bbox back to original frame coordinates for high-quality VLM processing
                        if original_frame is not None and scale_factor != (1.0, 1.0):
                            original_bbox = self._scale_bbox_to_original(bbox, scale_factor)
                            vlm_frame = original_frame
                            
                            if self.args.debug:
                                print(f"🔍 VLM Processing: Original bbox {bbox} → Scaled bbox {original_bbox}")
                                print(f"   Frame size: {frame.shape[:2]} → Original size: {original_frame.shape[:2]}")
                        else:
                            original_bbox = bbox
                            vlm_frame = frame
    ...
    def process_image_directory(self):
        ...
        for img_name in tqdm(image_files, desc="Processing images"):
            ...
            try:
                # Keep original image for VLM processing
                original_image = image.copy()
                scale_factor = self._calculate_scale_factor(image)
                
                processed_frame, frame_detections, new_violations = self.process_frame(
                    image, frame_count, original_image, scale_factor
                )
