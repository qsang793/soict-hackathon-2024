...
# Import utility functions for license plate handling
def read_license_plate(license_plate_crop):
    ...
    """
    # Initialize the OCR reader (first time only)
    if not hasattr(read_license_plate, "reader"):
        read_license_plate.reader = easyocr.Reader(['en'], gpu=torch.cuda.is_available())
    ...
    try:
        detections = read_license_plate.reader.readtext(license_plate_crop)
...
def write_license_plate_csv(results, output_path):
    """
    Write the license plate results to a CSV file.
...
try:
    ...
    print("Warning: Could not import LoLi_IEA or visualization utils. Image enhancement may not work.")


def parse_arguments():
    parser = argparse.ArgumentParser(description="Traffic Violation Detection with License Plate Recognition")
    ...
    parser.add_argument("--license_plate_model_path", type=str, default="license_plate_detector.pt")
    ...
    parser.add_argument("--output_license_plate_data", type=str, default="license_plate_data.csv")
    ...
    # License plate specific parameters
    parser.add_argument("--plate_conf", type=float, default=0.6,
                        help="Confidence threshold for license plate detection")
    parser.add_argument("--use_sort", action="store_true", 
                        help="Use SORT algorithm for tracking instead of built-in YOLO tracker")
    parser.add_argument("--max_age", type=int, default=5,
                        help="Maximum number of frames to keep track of an object after it's lost (for SORT)")
    parser.add_argument("--min_hits", type=int, default=3,
                        help="Minimum number of hits needed to start tracking an object (for SORT)")
    parser.add_argument("--iou_threshold", type=float, default=0.3,
                        help="IoU threshold for matching tracked objects (for SORT)")
    ...
    # Convert to TensorRT
    parser.add_argument("--use_tensorrt", action="store_true", help="Convert model to TensorRT for faster inference")
    parser.add_argument("--half_precision", action="store_true", help="Use FP16 for TensorRT (faster)")
    parser.add_argument("--tensorrt_workspace", type=int, default=8, help="Workspace size limit (GB) for TensorRT")
...
def infer_detect_track(model, source, conf=0.01, iou=0.7, device="cuda", tracker_type="bytetrack.yaml", persist=True):
...
def visualize_tracked_image_with_plates(image, detections, license_plates=None, trajectories=None, 
                                       trajectory_length=30, stop_line=None, stop_y=None, 
                                       green_zone=None, red_zone=None, violation_count=0, 
                                       is_red_phase=False, violations=None, 
                                       detection_method="combined", frame_count=0, debug=False):
    """
    Visualize detections with tracking information, zones, violations, and license plates
    """
    ...
    for i, (box, label, score, track_id, speed) in enumerate(detections):
        ...
        if has_violated:
            ...
            # Label with track ID and speed
            cv2.putText(image, f"ID:{track_id} {speed:.1f}km/h", (x1, y1 - 10), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        
        # If we have a license plate for this vehicle, draw it above the vehicle
        if license_plates is not None and track_id in license_plates:
            plate_info = license_plates[track_id]
            
            if 'text' in plate_info and plate_info['text'] is not None:
                # Display license plate number above vehicle
                plate_y_pos = max(0, y1 - 40)
                cv2.putText(image, f"Plate: {plate_info['text']}", 
                          (x1, plate_y_pos),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                
                # Draw license plate bounding box if available
                if 'bbox' in plate_info:
                    lp_x1, lp_y1, lp_x2, lp_y2 = map(int, plate_info['bbox'])
                    cv2.rectangle(image, (lp_x1, lp_y1), (lp_x2, lp_y2), (0, 255, 255), 2)
...
def calculate_speed(track, fps, pixels_per_meter=10):
    ...
    return min(speed_kmh, 150)  # Cap at reasonable maximum
...
def convert_to_tensorrt(model, model_path, half=False, workspace=8, dynamic=False, device=0, task=None):
    ...
    if task is None:
        if "vehicle" in model_path.lower():
            task = "detect"
        elif "license_plate" in model_path.lower():
            task = "detect"
    ...
    print(f"Converting {model_path} to TensorRT{'(FP16)' if half else ''} for task '{task}'")

    basename = os.path.splitext(model_path)[0]
    engine_path = f"{basename}_{task}.engine"
    
    if os.path.exists(engine_path):
        print(f"Found existing TensorRT engine at {engine_path}, using existing file")
        return engine_path
    ...
    try:
        ...
        if os.path.exists(engine_path):
            ...
            print(f"Engine not found after conversion")
            return None
    except Exception as e:
        print(f"Error converting to TensorRT: {str(e)}")
        print("Falling back to PyTorch model")
        return None


def check_tensorrt_compatibility():
    """Check compatibility with TensorRT"""
    if not torch.cuda.is_available():
        print("TensorRT requires NVIDIA GPU with CUDA. No CUDA GPU found.")
        return False
    ...
    try:
        ...
        print("TensorRT not installed. Please install tensorrt package.")
        return False
...
def process_video():
    ...
    if args.use_tensorrt:
        ...
        if not tensorrt_available:
            print("Switching to PyTorch mode as TensorRT is not available")
    ...
    output_license_plate_path = args.output_license_plate_data
    ...
    license_plate_model = YOLO(args.license_plate_model_path, task="detect")
    
    # Load day/night classification model
    daynight_model = YOLO(args.daynight_model_path, task="classify")
    
    # Initialize SORT tracker if needed
    if args.use_sort:
    ...
    if args.use_tensorrt and torch.cuda.is_available():
        ...
        if vehicle_engine_path:
            ...
            print("Loaded vehicle model from TensorRT engine")
        
        # TensorRT for license plate model
        license_plate_engine_path = convert_to_tensorrt(
            license_plate_model,
            args.license_plate_model_path, 
            half=args.half_precision,
            workspace=args.tensorrt_workspace,
            dynamic=args.tensorrt_dynamic,
            device=0 if device.type == "cuda" else "cpu",
        ...
        if license_plate_engine_path:
            license_plate_model = YOLO(license_plate_engine_path, task="detect")
            print("Loaded license plate model from TensorRT engine")
        
        # TensorRT for day/night model
        ...
        if daynight_engine_path:
            ...
            print("Loaded day/night classification model from TensorRT engine")
    ...
    if not args.disable_enhancement:
        try:
            ...
            print(f"Error loading enhancement models: {e}")
            print("Disabling image enhancement")
            light_enhancer = None
            NAFNet = None
            args.disable_enhancement = True
    else:
        light_enhancer = None
        NAFNet = None
    ...
    # Initialize license plate storage
    license_plates = {}  
    ...
    try:
        ...
        license_plate_model.predict(dummy_detect_frame, device=device, verbose=False)
        daynight_model.predict(dummy_classify_frame, device=device, verbose=False)
    except Exception as e:
        print(f"Warning: Error warming up models: {e}")
        print("Continuing despite warmup error...")
    ...
    if args.img_dir is None:
        ...
        try:
            ...
            with open(output_data_path, 'w') as f:
                f.write("frame,track_id,class,x1,y1,x2,y2,confidence,speed_kmh,violation,license_plate\n")
            ...
            while True:
                ...
                # Initialize license_plate_results for this frame
                license_plate_results[frame_count] = {}
                
                # Process frame
                try:
                    processed_frame, frame_detections, new_violations = process_integrated_frame(
                        frame, frame_count, vehicle_model, license_plate_model, daynight_model,
                        NAFNet, light_enhancer, device, enhancement_device,
                        args.conf, args.iou, args.plate_conf, trajectories, license_plates, 
                        license_plate_results, vehicle_data, fps, mot_tracker if args.use_sort else None,
                    ...
                    with open(output_data_path, 'a') as f:
                        for det in frame_detections:
                            track_id, cls, x1, y1, x2, y2, conf, speed, is_violation, plate_text = det
                            f.write(f"{frame_count},{track_id},{cls},{x1},{y1},{x2},{y2},{conf},{speed},{1 if is_violation else 0},{plate_text if plate_text else 'None'}\n")
            ...
            # Write license plate data to CSV
            write_license_plate_csv(license_plate_results, output_license_plate_path)
            ...
            print(f"License plate data saved to: {output_license_plate_path}")
        ...
        process_image_directory(args, device, enhancement_device, 
                               vehicle_model, license_plate_model, daynight_model, 
                               NAFNet, light_enhancer,
                               stop_line, stop_y, args.tolerance, green_zone, red_zone)
...
def process_integrated_frame(frame, frame_count, vehicle_model, license_plate_model, daynight_model, 
                          NAFNet, light_enhancer, device, enhancement_device, 
                          conf_threshold, iou_threshold, plate_conf_threshold,
                          trajectories, license_plates, license_plate_results,
                          vehicle_data, fps, mot_tracker=None,
                          memory_efficient=False, disable_enhancement=False, 
                          max_dimension=1280, trajectory_length=30,
                          tracker_type="bytetrack.yaml", stop_line=None, stop_y=None, 
                          tolerance=10, green_zone=None, red_zone=None, 
                          is_red_phase=False, violations=None, violation_count=0, 
                          detection_method="combined", violation_classes=None,
                          violation_cooldown=30, min_frames_in_green=3, 
                          interpolation_steps=5, debug=False):
    """Process a single frame with integrated license plate detection and recognition"""
    # Clear CUDA cache to prevent memory issues
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    try:
        ...
        if mot_tracker is not None:
            # Use SORT tracker
            # First detect vehicles with YOLO
            ...
            # Get bounding boxes, classes and scores
            if len(vehicle_results.boxes) > 0:
                ...
                for box, cls, score in zip(boxes, classes, scores):
                    # Add class and score to the detection
                    # Format: [x1, y1, x2, y2, score, class]
                    det = np.append(box, [score, cls])
                    detections.append(det)
                
                # If there are detections, update SORT
                if len(detections) > 0:
                    track_bbs_ids = mot_tracker.update(np.array(detections))
                else:
                    track_bbs_ids = np.empty((0, 6))  # Empty array with correct shape
            else:
                track_bbs_ids = np.empty((0, 6))  # Empty array if no detections
                
            yolo_track_results = None  # We're not using YOLO's built-in tracker
            has_tracking = True
            
        else:
            # Use YOLO's built-in tracker
            yolo_track_results = infer_detect_track(
                model=vehicle_model,
                source=detect_frame,
                conf=conf_threshold,
                iou=iou_threshold,
            ...
            track_bbs_ids = None  # Not using SORT
            has_tracking = hasattr(yolo_track_results.boxes, 'id') and yolo_track_results.boxes.id is not None
        
        # Now detect license plates
        license_plate_results_frame = license_plate_model.predict(
            detect_frame,
        ...
        # Process license plates if present
        if len(license_plate_results_frame.boxes) > 0:
            license_plate_boxes = license_plate_results_frame.boxes.xyxy.cpu().numpy()
            license_plate_scores = license_plate_results_frame.boxes.conf.cpu().numpy()
            license_plate_classes = license_plate_results_frame.boxes.cls.int().cpu().numpy()
            
            # Process each license plate
            for i, (lp_box, lp_score, lp_class) in enumerate(zip(license_plate_boxes, license_plate_scores, license_plate_classes)):
                # Format [x1, y1, x2, y2, score, class_id]
                license_plate = [lp_box[0], lp_box[1], lp_box[2], lp_box[3], lp_score, lp_class]
                
                # Match license plate to a vehicle
                if mot_tracker is not None:
                    # Using SORT tracker
                    vehicle_track_ids = track_bbs_ids
                    if len(vehicle_track_ids) > 0:
                        # Format expected by get_car: [xcar1, ycar1, xcar2, ycar2, car_id]
                        vehicle_track_ids_formatted = []
                        for track in vehicle_track_ids:
                            xcar1, ycar1, xcar2, ycar2, track_id = track[:5]
                            vehicle_track_ids_formatted.append([xcar1, ycar1, xcar2, ycar2, track_id])
                        
                        xcar1, ycar1, xcar2, ycar2, car_id = get_car(license_plate, vehicle_track_ids_formatted)
                    else:
                        xcar1, ycar1, xcar2, ycar2, car_id = -1, -1, -1, -1, -1
                else:
                    # Using YOLO's built-in tracker
                    if has_tracking and len(yolo_track_results.boxes) > 0:
                        # Format bounding boxes and IDs for get_car function
                        vehicle_track_ids_formatted = []
                        
                        boxes = yolo_track_results.boxes.xyxy.cpu().numpy()
                        track_ids = yolo_track_results.boxes.id.int().cpu().numpy()
                ...
                # If license plate is matched to a vehicle
                if car_id != -1:
                    # Crop license plate from image
                    x1, y1, x2, y2 = int(license_plate[0]), int(license_plate[1]), int(license_plate[2]), int(license_plate[3])
                    license_plate_crop = detect_frame[y1:y2, x1:x2, :]
                    ...
                    # Store license plate data if text was recognized
                    if license_plate_text is not None:
                        ...
                        # For CSV export
                        license_plate_results[frame_count][car_id] = {
        ...
        # Process tracked vehicles for violation detection and visualization
        frame_detections = []
        viz_detections = []
        ...
        if mot_tracker is not None:
            ...
            if len(track_bbs_ids) > 0:
                for bb_id in track_bbs_ids:
                    x1, y1, x2, y2, track_id, class_id = bb_id
                    ...
                    is_monitored = len(violation_classes) == 0 or class_id in violation_classes
                    ...
                    if is_red_phase and is_monitored and track_id not in violations:
                        ...
                        if violated:
                            ...
                            if debug:
                                ...
                                print(f"🚨 VIOLATION DETECTED! Vehicle {track_id}")
                    ...
                    plate_text = license_plates.get(track_id, {}).get('text', None)
                    ...
                    vehicle_data[track_id].append({
                        ...
                        "box": [x1, y1, x2, y2],
                        "label": class_id,
                        "score": 1.0,  # SORT doesn't provide confidence scores
                        "speed": speed,
                    ...
                    frame_detections.append((
                        ...
                        1.0, speed, is_violation or track_id in violations,
                        plate_text if plate_text else None
                    ))
                    
                    # Add to visualization list
                    viz_detections.append((
                        np.array([x1, y1, x2, y2]), class_id, 1.0, track_id, speed
                    ))
        else:
            # Using YOLO's built-in tracker
            if has_tracking and len(yolo_track_results.boxes) > 0:
                ...
                boxes = yolo_track_results.boxes.xyxy.cpu().numpy()
                track_ids = yolo_track_results.boxes.id.int().cpu().numpy()
                labels = yolo_track_results.boxes.cls.int().cpu().numpy()
                scores = yolo_track_results.boxes.conf.cpu().numpy()
        ...
        # Visualize detections with trajectories, zones, license plates and stop line
        result_frame = visualize_tracked_image_with_plates(
            original_frame, viz_detections, license_plates, trajectories, trajectory_length,
    ...
    cv2.putText(result_frame, f"Plates: {len(license_plates)}", (10, 120), 
...
def process_image_directory(args, device, enhancement_device, 
    ...
    for img_name in tqdm(image_files, desc="Processing images"):
        ...
        try:
            processed_frame, frame_detections, new_violations = process_integrated_frame(
                image, frame_count, vehicle_model, license_plate_model, daynight_model,
                NAFNet, light_enhancer, device, enhancement_device,
                args.conf, args.iou, args.plate_conf, trajectories, license_plates,
                license_plate_results, vehicle_data, 30,  # Assuming 30 fps
                mot_tracker,
                args.memory_efficient, args.disable_enhancement, args.max_dimension,
                args.trajectory_length, args.tracker,
            ...
            for det in frame_detections:
                track_id, cls, x1, y1, x2, y2, conf, speed, is_violation, plate_text = det
                plate_info = f" {plate_text}" if plate_text else ""
                results.append(f"{img_name} {cls} {x1/image.shape[1]} {y1/image.shape[0]} {(x2-x1)/image.shape[1]} {(y2-y1)/image.shape[0]} {conf} {1 if is_violation else 0}{plate_info}")
