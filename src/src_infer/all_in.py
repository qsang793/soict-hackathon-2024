...
# Enhanced tracker implementation to replace ByteTrack
class EnhancedTracker:
    """Enhanced IOU + motion-based tracker for vehicle tracking"""
    def __init__(self, iou_threshold=0.5, max_age=30, max_displacement=100):
        self.tracks = {}  # id -> {"box": box, "age": age, "history": [positions], "velocity": [vx, vy]}
        self.next_id = 1
        self.iou_threshold = iou_threshold
        self.max_age = max_age
        self.max_displacement = max_displacement
    
    def update(self, boxes, labels=None, scores=None):
        """Update tracker with new detections"""
        if boxes.size == 0 or len(boxes) == 0:
            return []
        
        # Ensure labels and scores are available
        if labels is None:
            labels = [0] * len(boxes)
        if scores is None:
            scores = [1.0] * len(boxes)
            
        # If no tracks exist yet, create new ones for all boxes
        if not self.tracks:
            for i, box in enumerate(boxes):
                self.tracks[self.next_id] = {
                    ...
                    "age": 0,
                    "history": [self._get_center(box)],
                    "velocity": [0, 0]
                }
                self.next_id += 1
            return [(id, box, labels[i], scores[i]) for i, (id, box) in enumerate(zip(self.tracks.keys(), boxes))]
        ...
        for track_id, track in self.tracks.items():
            if len(track["history"]) >= 2:
                ...
                track["velocity"][0] = 0.7 * track["velocity"][0] + 0.3 * vx
                track["velocity"][1] = 0.7 * track["velocity"][1] + 0.3 * vy
        
        # Calculate cost matrix based on IoU and position
        cost_matrix = np.zeros((len(self.tracks), len(boxes)))
        track_ids = list(self.tracks.keys())
        
        for t, track_id in enumerate(track_ids):
            track = self.tracks[track_id]
            for d, detection in enumerate(boxes):
                # IoU cost
                iou_cost = 1.0 - self._calculate_iou(track["box"], detection)
                
                # Position cost (if we have history)
                pos_cost = 1.0
                if len(track["history"]) > 0:
                    det_center = self._get_center(detection)
                    # Predicted position
                    pred_x = track["history"][-1][0] + track["velocity"][0]
                    pred_y = track["history"][-1][1] + track["velocity"][1]
                    
                    # Distance between prediction and detection
                    dist = np.sqrt((pred_x - det_center[0])**2 + (pred_y - det_center[1])**2)
                    pos_cost = min(1.0, dist / self.max_displacement)
                
                # Combined cost (70% IoU, 30% position)
                cost_matrix[t, d] = 0.7 * iou_cost + 0.3 * pos_cost
        ...
        try:
            ...
            for t in range(len(track_ids)):
                ...
                if best_match >= 0:
                    ...
                    detection_indices.append(best_match)
        ...
        for t, d in zip(track_indices, detection_indices):
            ...
            # Only consider as a match if the cost is below threshold
            if cost_matrix[t, d] < 0.7:  # Combined threshold
                matched_indices.append((track_id, d))
        ...
        for track_id, det_idx in matched_indices:
            box = boxes[det_idx]
            label = labels[det_idx] if det_idx < len(labels) else 0
            score = scores[det_idx] if det_idx < len(scores) else 1.0
            ...
            result.append((track_id, box, label, score))
        ...
        for track_id in unmatched_tracks:
            ...
            # Optionally predict and keep track
            elif self.tracks[track_id]["age"] <= 5:  # Short-term prediction
                # Use velocity to predict new position
                last_pos = self.tracks[track_id]["history"][-1]
                vx, vy = self.tracks[track_id]["velocity"]
                
                # Update history with predicted position
                pred_x = last_pos[0] + vx
                pred_y = last_pos[1] + vy
                self.tracks[track_id]["history"].append((pred_x, pred_y))
                
                # Update box
                box = self.tracks[track_id]["box"].copy()
                cx, cy = (box[0] + box[2]) / 2, (box[1] + box[3]) / 2
                w, h = box[2] - box[0], box[3] - box[1]
                new_cx, new_cy = cx + vx, cy + vy
                
                new_box = [new_cx - w/2, new_cy - h/2, new_cx + w/2, new_cy + h/2]
                self.tracks[track_id]["box"] = np.array(new_box)
...
def visualize_tracked_image(image, detections, tracker_data=None, trajectory_length=30):
    ...
    for i, (box, label, score, track_id) in enumerate(detections):
...
def process_video():
    ...
    # Initialize our custom tracker (not using ByteTrack)
    tracker = EnhancedTracker(iou_threshold=0.5, max_age=30, max_displacement=100)
            ...
            while True:
                ...
                try:
                    processed_frame, frame_detections = process_frame(
                        ...
                        args.conf, args.iou, tracker, trajectories, vehicle_data, fps,
...
def process_frame(frame, frame_count, vehicle_model, daynight_model, NAFNet, light_enhancer,
                 ...
                 tracker, trajectories, vehicle_data, fps, memory_efficient=False,
                 disable_enhancement=False, max_dimension=1280):
    ...
    try:
        ...
        # Run vehicle detection (no tracking - we use our own tracker)
        ...
        # Process detections with our custom tracker
        if len(detections.boxes) > 0:
            ...
            for track_id, box, label, score in tracked_objects:
                # Get track from tracker data
                if track_id not in trajectories:
                    ...
                    trajectories[track_id]["history"] = tracker.tracks[track_id]["history"]
                ...
                viz_detections.append((
                    box, label, score, int(track_id)
                ))
            
            # Visualize detections with trajectories
            result_frame = visualize_tracked_image(original_frame, viz_detections, trajectories)
        else:
            # No detections
    ...
    for img_name in tqdm(image_files, desc="Processing images"):
        ...
        try:
            processed_frame, frame_detections = process_frame(
                ...
                NAFNet, light_enhancer, device, enhancement_device,
                args.conf, args.iou, tracker, trajectories, vehicle_data, 30,  # Assuming 30 fps
                args.memory_efficient, args.disable_enhancement, args.max_dimension
            )
