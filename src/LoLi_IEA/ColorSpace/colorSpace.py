...
def RgbToHsv(RGB):
    RgbNormalized = (
        RGB / 255.0
    )  # In this part what I do is normalize the RGB channel and divide it
    ...
    rMax = np.logical_and(
        R == vMax, C > 0
    )  # Hue calculation depending on the value range
...
def YcbcrTorgb(YCBCR):
    xform = np.array([[1, 0, 1.403], [1, -0.344, -0.714], [1, 1.773, 0]])
