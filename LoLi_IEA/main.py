...
def Enhancement(imgx, device):
    ...
    valComponentTensor = (
        torch.from_numpy(valComponent).float().to(device).view(1, 1, rows, columns)
    )
...
if __name__ == "__main__":
    ...
    enhancementSet = (
        "/home/manhckv/manhckv/soict/data/day_night_classify/train/nighttime"
    )
    ...
    for i in progress_bar:
        ...
        cv2.imwrite(os.path.join(save_folder, dataset.image_files[i]), imgsave * 255)

        # Print the processing time for the current image
        elapsed_time = time.time() - start_time
        progress_bar.set_description(
            "Image {0} enhanced in {1:.2f} seconds".format(i + 1, elapsed_time)
        )
